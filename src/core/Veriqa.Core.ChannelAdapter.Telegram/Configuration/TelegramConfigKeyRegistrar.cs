...
/// <summary>
/// Declares the Telegram channel's configuration keys into the schema of the deployment and states how
/// their core level is read (SPEC-012 §10.6). Registered by the channel's own <c>AddTelegram()</c>,
...
/// </summary>
internal sealed class TelegramConfigKeyRegistrar : IRegisterConfigKeys
{
    /// <summary>
    /// Global Telegram channel settings.
    /// </summary>
    private readonly IOptionsMonitor<TelegramOptions> _options;

    /// <summary>
    /// Creates the registrar of the Telegram channel keys.
    /// </summary>
    /// <param name="options">Global Telegram channel settings.</param>
    public TelegramConfigKeyRegistrar(IOptionsMonitor<TelegramOptions> options)
    {
    ...
    /// (<see cref="TelegramConfigKeys.Catalog"/>), which <c>AddTelegram()</c> registers. What is left here is
    ...
    public void Register(IConfigBindings bindings, IConfigCoreBindings coreBindings)
    {
        ...
        coreBindings.RegisterCore(
            TelegramConfigKeys.Credentials,
