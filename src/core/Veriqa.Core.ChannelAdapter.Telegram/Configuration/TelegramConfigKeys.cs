...
/// <summary>
/// Canonical resolver keys owned by the Telegram channel (TASK-040, SPEC-012 §10.6). The channel owns
/// the CATALOG its keys are declared into and binds their core level itself
/// (<see cref="TelegramConfigKeyRegistrar"/>), so no shared registry has to list the channels the
/// product happens to ship.
/// <para>
/// The catalog is registered where the channel is COMPOSED — <c>AddTelegram()</c> — and that is what
...
/// </para>
/// </summary>
public static class TelegramConfigKeys
{
    /// <summary>
    ...
    /// <summary>
    /// Telegram tenant-credential group of the layer (<c>Channels.telegram.Credentials</c>).
    /// </summary>
    public static ConfigKey<TelegramTenantCredentials> Credentials { get; } =
        ChannelConfigKeys.CredentialKey<TelegramTenantCredentials>(Declared, ChannelTypes.Telegram);

    /// <summary>
    /// Declarations of this channel — what <c>AddTelegram()</c> registers into the container, and what
